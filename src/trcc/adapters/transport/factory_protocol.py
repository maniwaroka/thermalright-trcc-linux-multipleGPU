...
class WindowsScsiProtocol(DeviceProtocol):
    ...
    def _get_transport(self):
        ...
        if self._transport is None or self._transport._handle is None:
            from .bridge_windows import WindowsScsiTransport
    ...
    def _do_handshake(self) -> Optional[HandshakeResult]:
        ...
        from .adapter_scsi import ScsiDevice, _POST_INIT_DELAY
...
class HidProtocol(UsbProtocol):
    ...
    def _do_handshake(self) -> Optional[HandshakeResult]:
        ...
        from .adapter_hid import HidDeviceType2, HidDeviceType3
    ...
    def send_image(self, image_data: bytes, width: int, height: int) -> bool:
        def _do_send() -> bool:
            from .adapter_hid import HidDeviceManager
            self._ensure_transport()
            assert self._transport is not None
...
class DeviceProtocolFactory:
    ...
    def create_usb_transport(vid: int, pid: int):
        ...
        from .adapter_hid import HIDAPI_AVAILABLE, PYUSB_AVAILABLE
        if PYUSB_AVAILABLE:
            from .adapter_hid import PyUsbTransport
            return PyUsbTransport(vid, pid)
        elif HIDAPI_AVAILABLE:
            from .adapter_hid import HidApiTransport
