...
class TestType2Handshake:
    ...
    def test_handshake_timing(self):
        ...
        dev = HidDeviceType2(transport)
        with patch("trcc.adapters.transport.adapter_hid.time.sleep") as mock_sleep:
            dev.handshake()
...
class TestPyUsbTransport:
    ...
    def test_open_find_device(self):
        ...
        with patch("trcc.adapters.transport.adapter_hid.usb.core.find", return_value=mock_dev):
            t = PyUsbTransport(0x0416, 0x5302)
            t.open()
    ...
    def test_close_releases_interface(self):
        ...
        mock_dev.is_kernel_driver_active.return_value = False
        with patch("trcc.adapters.transport.adapter_hid.usb.core.find", return_value=mock_dev), \
             patch("trcc.adapters.transport.adapter_hid.usb.util.claim_interface"), \
             patch("trcc.adapters.transport.adapter_hid.usb.util.release_interface") as mock_release, \
             patch("trcc.adapters.transport.adapter_hid.usb.util.dispose_resources") as mock_dispose:
            t = PyUsbTransport(0x0416, 0x5302)
            t.open()
            t.close()
    ...
    def test_context_manager(self):
        ...
        with patch("trcc.adapters.transport.adapter_hid.usb.core.find", return_value=mock_dev), \
             patch("trcc.adapters.transport.adapter_hid.usb.util.claim_interface"), \
             patch("trcc.adapters.transport.adapter_hid.usb.util.release_interface"), \
             patch("trcc.adapters.transport.adapter_hid.usb.util.dispose_resources"):
...
class TestHidApiTransport:
    ...
    def test_import_error_when_hidapi_missing(self):
        ...
        import trcc.adapters.transport.adapter_hid as mod
...
class TestFindHidDevices:
    ...
    def test_pyusb_discovery(self):
        ...
        with patch("trcc.adapters.transport.adapter_hid.usb.core.find") as mock_find, \
             patch("trcc.adapters.transport.adapter_hid.usb.util.get_string", return_value="ABC123"):
